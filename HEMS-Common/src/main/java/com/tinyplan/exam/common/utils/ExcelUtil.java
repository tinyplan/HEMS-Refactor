...
public class ExcelUtil {
    ...
    private static final Map<String, String> INVIGILATOR_HEADER_ALIAS;

    static {
        SITE_HEADER_ALIAS = new HashMap<>(4);
        ...
        SCORE_HEADER_ALIAS = new HashMap<>(3);
        SCORE_HEADER_ALIAS.put("准考证号", "candidateNo");
        SCORE_HEADER_ALIAS.put("考生姓名", "realName");
        SCORE_HEADER_ALIAS.put("考试成绩", "score");

        INVIGILATOR_HEADER_ALIAS = new HashMap<>(2);
        INVIGILATOR_HEADER_ALIAS.put("姓名", "realName");
        INVIGILATOR_HEADER_ALIAS.put("手机号", "contact");
    }
    ...
    public static List<ScoreDTO> readScoreExcel(File scoreExcel) {
        ...
        scoreList.forEach(dto -> dto.setExamNo(examNo));
        return scoreList;
    }

    public static List<InvigilatorDTO> readInvigilatorExcel(File invFile) {
        if (!ExcelFileUtil.isXlsx(invFile)) {
            throw new BusinessException(ResultStatus.RES_INVALID_FILE_TYPE);
        }
        ExcelReader reader = new ExcelReader(invFile, "Sheet1");
        reader.setHeaderAlias(INVIGILATOR_HEADER_ALIAS);
        return reader.read(0, 1, InvigilatorDTO.class);
    }
